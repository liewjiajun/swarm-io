# 02 - Shared Types and Constants

## Overview
Define all shared TypeScript types used by both client and server. These types represent game entities, network messages, and game state.
...
2. Types are importable from both client and server using `import { PlayerState } from '@swarm-io/shared'`
3. Constants provide all balance values needed for game logic
4. Utility functions handle common vector math operations
5. Wave schedule defines enemy progression for first 6 minutes
6. XP curve provides increasing difficulty as expected
